import React from 'react';
import { assetPath } from '../../utils/assetPath';
import './Login.css';
import { useAuth } from '../../context/AuthContext';
import OutlinedTitle from '../../components/heading/OutlinedTitle';

const Login: React.FC = () => {
    const { loginWithGoogle } = useAuth();

    const handleLogin = async () => {
        try {
            await loginWithGoogle();
            // The AuthProvider's useEffect will update state. 
            // We'll handle redirection in a separate useEffect or here after state updates.
        } catch (error) {
            console.error("Login Error:", error);
        }
    };

    // Handle redirection after auth state changes is now handled by PublicRoute wrapper in App.tsx

    return (
        <div className="login-page">
            <div className="login-card">
                <div className="login-logo-container">
                    <img 
                        src={assetPath('/assets/logos/ZCOER-Logo-White.png')}
                        alt="ZCOER Logo" 
                        className="login-logo"
                        draggable={false}
                    />
                </div>

                <div className="login-header">
                    <OutlinedTitle 
                        text="ARENA" 
                        fillColor="#f0ff00" 
                        outlineColor="#000" 
                        hasGrain={true}
                        className="login-title"
                    />
                    <p>Enter the Talentron Arena</p>
                </div>

                <div className="login-actions">
                    <button className="google-login-btn" onClick={handleLogin}>
                        <svg className="google-icon" viewBox="0 0 24 24">
                            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                            <path fill="#FBBC05" d="M5.84 14.1c-.22-.66-.35-1.36-.35-2.1s.13-1.44.35-2.1V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94L5.84 14.1z"/>
                            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.66l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"/>
                        </svg>
                        Continue with Google
                    </button>
                    
                    <a href="/" className="back-home">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                            <line x1="19" y1="12" x2="5" y2="12"></line>
                            <polyline points="12 19 5 12 12 5"></polyline>
                        </svg>
                        Return to Home
                    </a>
                </div>

                <div className="login-footer">
                    <p>By logging in, you agree to the Arena's Charter</p>
                </div>
            </div>
            
            <div className="login-bg-text">TLRN</div>
        </div>
    );
};

export default Login;
